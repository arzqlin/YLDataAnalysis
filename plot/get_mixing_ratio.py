...
def get_mixing_ratio():
    ...
    for j in range(1, 30):  
        ...
        YL_median[chem] = np.float64(sh.cell_value(17,j))

    sorted_ratio = sorted(YL_median.items(), key=lambda x: x[1], reverse = True)

    chemical = list(zip(*sorted_ratio))[0]
    ra = list(zip(*sorted_ratio))[1]
    ...
    plt.ylabel('Mxing Ratio (ppbv)',fontsize = 12, fontweight = 'bold',labelpad = 4)
    plt.grid(True,color='#404040',alpha = 0.05, linestyle='-',linewidth = 1 )
    plt.tight_layout(w_pad = 2, h_pad = 2)

    now = datetime.now()
    plt.savefig('./output3/Percentage_YL_{}.png'.format(now.strftime("%Y-%m-%d_%H-%M")))